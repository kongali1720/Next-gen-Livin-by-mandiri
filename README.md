...
[![Target](https://img.shields.io/badge/Target-Bank%20Mandiri-yellow?style=for-the-badge)]()
...
```mermaid
sequenceDiagram
    participant User as Livin' User
    participant App as Livin' Super App
    participant Gateway as Mandiri API Gateway
    participant Vault as Secure Token Vault

    User->>App: Request Akses Fitur Baru
    App->>Gateway: API Request (JWT)
    Gateway->>Vault: Validasi & Dekripsi
    Vault-->>Gateway: Verified
    Gateway-->>App: Real-Time Response
    App-->>User: Success UI
```
